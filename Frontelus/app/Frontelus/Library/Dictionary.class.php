...
final class Dictionary
{
    ...
    public function deleteDefinition_wordBuffer($word, $definition)
    {
        if (isset($this->WordBuffer[$word]))
        {
            $key = array_search($definition, $this->WordBuffer[$word]);  
            if ($key !== false)
            { 
                unset($this->WordBuffer[$word][$key]);
                $this->WordBuffer[$word] =  array_values($this->WordBuffer[$word]);
            } 
        }
    }
    
    public function deleteDefinition_word($definition)
    {
        $key = array_search($definition, $this->Word); 
        if ($key !== FALSE)
        {
            $this->Word[$key] = NULL;
        }
    ...
    public function indexExists_word($index)
    {
        if (isset($this->Word[$index]))
        {
            return TRUE;
        }
        return FALSE;
